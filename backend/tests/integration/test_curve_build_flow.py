...
@pytest.mark.asyncio
async def test_seed_quotes_and_build_curve(client: AsyncClient, db_session: AsyncSession):
    quotes = [
    ...
    for quote in quotes:
        resp = await client.post("/api/v1/market-data/quotes", json=quote)
        assert resp.status_code == 201

    build_resp = await client.post("/api/v1/curves/build", json={"as_of_date": "2026-01-15"})
    ...
    get_resp = await client.get(f"/api/v1/curves/{curve['id']}")
...
async def test_build_curve_with_no_quotes_returns_422(
    ...
    resp = await client.post("/api/v1/curves/build", json={"as_of_date": "2026-01-15"})
