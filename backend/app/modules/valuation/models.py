...
class Position(Base):
    ...
    net_volume: Mapped[Decimal] = mapped_column(Numeric(18, 4), nullable=False)
    avg_fixed_price: Mapped[Decimal] = mapped_column(Numeric(18, 6), nullable=False)
    as_of_date: Mapped[date] = mapped_column(Date, nullable=False)
...
class ValuationResult(Base):
    ...
    as_of_date: Mapped[date] = mapped_column(Date, nullable=False)
    curve_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("forward_curves.id"), nullable=False)
    mtm_value: Mapped[Decimal] = mapped_column(Numeric(20, 6), nullable=False)
